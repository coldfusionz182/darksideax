...
export default async function handler(req, res) {
  ...
  try {
    ...
    let query = supabase
      .from('threads')
      .select(
        'id, title, tag, author, content, created_at, replies, views, last_post_user, last_post_time, section, marketplace_status, is_locked, approved, config_request_status, download_url, completed_by'
      )
      .eq('approved', true)
